...
2. 假設頁面 ( MPS ) 大小為 4 KB，傳輸資料大小為 8 KB，當前資料量大於 MPS，因此需要使用 PRP2 Entry 描述第二筆資料的記憶體位址空間：

	- **PRP1 Entry**
	    - 描述指向第一筆記憶體位址 `0x0000001:09BFD000`。
	- **PRP2 Entry
	    - 描述剩餘傳輸的記憶體位址 `0x0000001:09BFE000`。
...
3.  假設頁面 ( MPS ) 大小為 4 KB，傳輸資料大小為 16KB，當前資料量大於 MPS，因此需要使用PRP2 Entry 描述多筆資料記憶體位址空間：
	...
	- **備註**
		...
		- 從主機端發出的命令來看，記憶體的確沒有對齊 ( SPEC 暫時未看到相關說明 )
