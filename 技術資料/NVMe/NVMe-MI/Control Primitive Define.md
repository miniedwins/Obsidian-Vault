...
## 運作功能
當管理控制器（Management Controller）需要對端點（Management Endpoint）下達某種即時控制時，就會送出一個 Control Primitive 訊息。該訊息會放在 **NVMe-MI Message Fields** 欄位中的 
**NVMe-MI Message Type** 來表示傳遞的訊息是 Control Primitive Message。
