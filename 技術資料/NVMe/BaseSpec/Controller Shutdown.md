...
- **RTD3進入延遲**：這是指控製器進入低功耗狀態（RTD3）的時間。在關閉操作完成之前，主機需要等待至少RTD3進入延遲時間。如果**RTD3進入延遲**值為0h，那麼主機至少應該等待1秒鐘。
    
- **不建議通過CC.EN欄位停用控製器**：通過這種方式停用控製器會導致控製器重設，這可能會影響完成關閉處理所需的時間。
    
- **關閉處理進行時**：在關閉處理進行時，控制器會中止任何命令，並返回 "Power Loss Notification" 的狀態程式碼。

當 `CSTS.ST=0` 被清除為 "0" 時，以及 `CSTS.SHST=10b`，表明控製器不再處於活動狀態，已完成所有必要的處理。 無論 CC.EN 的值如何，這個時候斷電都是安全的。

控製器報告關閉處理完成後，如何重新啟動控製器，讓控制起可以執行命令 : 

- **如果CC.EN被設定為‘1’**：那麼必須執行**控製器重設**，即將**CC.EN**從‘1’清除為‘0’（停用控製器），然後再重新啟用它。
    
- **如果CC.EN已經清除為‘0’**：那麼控製器必須被**啟用**，即將**CC.EN**從‘0’設定為‘1’（啟用控製器）。
